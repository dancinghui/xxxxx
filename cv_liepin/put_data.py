...
class DisPageStore(page_store.LPCVStore):
...
class CVBin2DB(Bin2DB):
    ...
    def parse_name(self, n):
        m = re.match(r'cv_liepin\.([a-zA-Z0-9]+)\.(\d+)', n)
        if m:
    ...
    def get_url(self, jdid):
        return 'https://lpt.liepin.com/resume/showresumedetail/?res_id_encode={}&isBatch=0'.format(jdid)
