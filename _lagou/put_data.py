...
class DisLGPageStore(page_store.PageStoreLG):
...
class JDLGBin2DB(Bin2DB):
    def __init__(self):
        Bin2DB.__init__(self)
        self.pagestore = DisLGPageStore()

    def parse_name(self, n):
        m = re.match(r'jd_lagou\.(\d+)\.(\d+)', n)
        if m:
    ...
    def get_url(self, jdid):
        return "http://www.lagou.com/jobs/%s.html" % jdid
