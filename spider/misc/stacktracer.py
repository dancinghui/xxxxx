...
def stacktraces():
    ...
    for threadId, stack in sys._current_frames().items():
        code.append("\n# ThreadID: %s name: %s" % (threadId, Runtime.get_thread_name(threadId)))
...
class TraceDumper(threading.Thread):
    ...
    def run(self):
        Runtime.set_current_thread_name('stacktracer')
    ...
    def stacktraces(self):
        fout = file(self.fpath+".tmp", "wb+")
        try:
            fout.write(stacktraces())
        finally:
            fout.close()
            shutil.copyfile(self.fpath+".tmp", self.fpath)
