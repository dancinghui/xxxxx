...
def test_proxy(p):
    url = "http://qy.chinahr.com/buser/login?"
...
def extract_content(doc):
    ...
    find = htmlfind(doc, '<div class="login">', 0)
    if find:
        # print htmlfind.remove_tag(find.get_text(),1)
        return True
...
if __name__ == '__main__':
    ...
    _checked_file = 'checked_proxy'
    if os.path.exists(_checked_file):
        os.remove(_checked_file)

    with open(_checked_file, 'wb')  as f:
