...
# !/usr/bin/env python
# -*- coding:utf8 -*-
from _gk_chsi.runner import FullJobRunner, gen_jobs_params

from _gk_chsi.accounts import AccountManager, provinces


def run_fun_ah(ac):
    runner = FullJobRunner(
        gen_jobs_params(ac['accounts'], ac['name'], ac['prefix'], spec=True, detail=True, school=False,
                        kldms=['5', '1'],
...
if __name__ == '__main__':
    ...
    # ]
    proxies = []
    am = AccountManager(src='../accounts.dat')
    ...
    runner = FullJobRunner(
        gen_jobs_params(ac['accounts'], ac['name'], ac['prefix'], spec=False, detail=True, school=False,
                        kldms=['5', '1'],
                        spec_parser=True, detail_parser=True, spec_r=False, detail_r=True,
                        sch_parser=False))
    runner.run()
