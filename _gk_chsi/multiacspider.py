...
from court.util import save_file, Captcha, remove_file
...
class BaseGkChsiFsxSpider(SleepSpider):
    ...
    def login(self):
        ...
        print 'logging in as %s' % self.account['username']
...
class MaChsiSpider(BaseGkChsiFsxSpider):
    def __init__(self, threadcnt, account, tag, proxy=None, sleep=0.0, captcha_limit=50000000, sleep_max=5,
                 job_tag='', post_kldm=True):
        if not isinstance(account, list):
            account = [account]
        super(MaChsiSpider, self).__init__(threadcnt, account[0], '%s%s' % (tag, job_tag), proxy, sleep, captcha_limit,
                                           sleep_max, account[0]['ua'], post_kldm)
        self.__cur_kldm = None
        self.__cur_bkcc = None
        self.__acc_arr = account
        self.__acc_lock = threading.RLock()
        self.__cur_acc_index = 0

    def on_time_out(self):
        with self.__acc_lock:
            self.logout()
            c = len(self.__acc_arr) - 1
            self.__acc_arr[self.__cur_acc_index]['timeout'] = True
            self.__cur_acc_index = (self.__cur_acc_index + 1) % len(self.__acc_arr)
            while c > 0 and (self.__acc_arr[self.__cur_acc_index]['timeout'] or self.__acc_arr[self.__cur_acc_index][
                'captcha']):
                self.__cur_acc_index = (self.__cur_acc_index + 1) % len(self.__acc_arr)
                c -= 1

            if c > 0:
                self.account = self.__acc_arr[self.__cur_acc_index]
                self.select_user_agent(self.account['ua'])
                self.login()
                if None != self.__cur_kldm and self.post_kldm:
                    self.post_kldm_bkcc_for_session(self.__cur_kldm, self.__cur_bkcc)
            else:
                self._shutdown = True

    def post_kldm_bkcc_for_session(self, kldm, bkcc):
        self.__cur_bkcc = bkcc
        self.__cur_kldm = kldm
        super(MaChsiSpider, self).post_kldm_bkcc_for_session(kldm, bkcc)

    def on_captcha(self):
        with self.__acc_lock:
            self.logout()
            c = len(self.__acc_arr) - 1
            self.__acc_arr[self.__cur_acc_index]['captcha'] = True
            self.__cur_acc_index = (self.__cur_acc_index + 1) % len(self.__acc_arr)
            while c > 0 and (self.__acc_arr[self.__cur_acc_index].get('timeout', False) or self.__acc_arr[
                self.__cur_acc_index].get('captcha', False)):
                self.__cur_acc_index = (self.__cur_acc_index + 1) % len(self.__acc_arr)
                c -= 1
...
class ChsiSpider(MaChsiSpider):
    def __init__(self, threadcnt, account, tag, proxy=None, sleep=0.0, captcha_limit=50000000, sleep_max=5,
                 seeds='detail_seeds', recover=False, year='15', bkccs=None, kldms=None,
                 job_tag='', spider_type='detail', post_kldm=True):
        super(ChsiSpider, self).__init__(threadcnt, account, tag, proxy, sleep, captcha_limit,
                                         sleep_max, post_kldm=post_kldm, job_tag=job_tag)
        if kldms is None:
            kldms = ['5', '1']
        if bkccs is None:
            bkccs = ['1', '2']
...
class ChsiDetailSpider(ChsiSpider):
    def __init__(self, threadcnt, account, tag, proxy=None, sleep=0.0, captcha_limit=50000000, seeds='detail_seeds',
                 recover=False, sleep_max=5, year='15', bkccs=None, kldms=None, job_tag=''):
        super(ChsiDetailSpider, self).__init__(threadcnt, account, tag, proxy, sleep, captcha_limit, sleep_max,
                                               seeds=seeds, recover=recover, year=year, bkccs=bkccs, kldms=kldms,
...
class ChsiSchoolSpider(ChsiSpider):
    def __init__(self, threadcnt, account, tag, proxy=None, sleep=0.0, captcha_limit=50000000, seeds='school.seeds',
        ...
        super(ChsiSchoolSpider, self).__init__(threadcnt, account, tag, proxy, sleep, captcha_limit, sleep_max,
                                               seeds=seeds, recover=recover, year=year, bkccs=bkccs, kldms=kldms,
                                               job_tag=job_tag, spider_type='sch', post_kldm=False)
