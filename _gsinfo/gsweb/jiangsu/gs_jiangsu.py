...
class SearchGSWebJiangsu(SearchGSWeb):
    ...
    def search_company(self, kw):
        headers = {'Referer': "http://www.jsgsj.gov.cn:58888/province/queryResultList.jsp",
                   "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
                   "X-Requested-With": "XMLHttpRequest"}
        infourl = "http://www.jsgsj.gov.cn:58888/province/infoQueryServlet.json?queryCinfo=true"
        captcha = "" #random.randrange(-10, 82, 1)
        out = []
        names = ""
        while True:
            si = self.request_url(infourl, data={"name": kw, "verifyCode": captcha}, headers=headers)
...
class RunJiangsu(Spider):
    ...
    def __init__(self):
        ...
        self.saver = RunJiangsu.Saver()
        self.is_debug = True
        if self.is_debug:
            Spider.__init__(self, 1)
            self.gsweb = SearchGSWebJiangsu(self.saver)
        else:
            self.proxies_dict = []
    ...
    def init_obj(self):
        ...
        gsweb = SearchGSWebJiangsu(self.saver)
...
if __name__ == '__main__':
    spider.util.use_utf8()
    gsweb = SearchGSWebJiangsu(RunJiangsu.Saver())
    gsweb.search_company("江苏科技")
