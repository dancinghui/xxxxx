...
class RunGuangdong(Spider):
    ...
    class Saver(object):
        def __init__(self):
            ...
            self.fs_QyxyDetail = FileSaver("gsinfo_guangdong_QyxyDetail_gz3.txt")
            self.fs_GSpublicityList = FileSaver("gsinfo_guangdong_GSpublicityList_gz3.txt")
            self.fs_entityShow = FileSaver("gsinfo_guangdong_entityShow_gz3.txt")
    ...
    def __init__(self):
        ...
        self.gswebs = {}
        #已经获取详情成功的关键字
        self.success_name = FileSaver("already_guangzhou_inc_name3.txt")
        #已经获取详情成功的查询到的公司名
        self.succes_query_name = FileSaver("already_guangzhou_query_inc_name3.txt")
        #页面提示无法显示的公司列表信息out[]
        self.not_show_save = FileSaver("guangzhou_not_show_out3.txt")
        #查不到内容的关键字
        self.query_none_kw = FileSaver("already_query_none_kw.txt")
    ...
    def init_spider_url(self):
        with open("already_guangzhou_inc_name3.txt", "r") as f:
            i = 0
            for name in f:
                filter_name.add(name.strip().decode("utf-8"))
                i += 1
            print "init success keywords . ", i

        with open("already_query_none_kw.txt", "r") as f:
            i = 0
            for name in f:
                filter_name.add(name.strip().decode("utf-8"))
                i += 1
            print "init already_query_none_kw . ", i

        with open("already_guangzhou_query_inc_name3.txt", "r") as f:
            i = 0
            for name in f:
                filter_query_name.add(name.strip().decode("utf-8"))
                i += 1
            print "init success query company name . ", i
    ...
    def run_job(self, jobid):
        ...
        gsweb.select_user_agent("=Mozilla/5.0 (Windows NT "+str(random.randrange(1, 99, 1))+".0; WOW64) AppleWebKit/"+str(random.randrange(100, 999, 1))+"."+str(random.randrange(1, 99, 1))+" (KHTML, like Gecko) Chrome/"+str(random.randrange(1, 99, 1))+".0.2311."+str(random.randrange(100, 999, 1))+" Safari/"+str(random.randrange(100, 999, 1))+"."+str(random.randrange(1, 99, 1))+" LBBROWSER")
        tp = jobid["type"]
        cnt = jobid.get("cnt")
