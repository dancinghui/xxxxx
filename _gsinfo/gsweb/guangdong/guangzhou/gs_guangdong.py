...
jsluids = ["__jsluid=1977972640d0ea79e91d88084c8ccbdc", "__jsluid=c18a18530c89164c9eabf0542fd55e71", "__jsluid=a94a71cbeaae737e16dda56d08ea04f4"]
...
#############################  GUANGZHOU  new ##########################################################
    ...
    def get_guangzhou(self, cnt, cname, url, regist_code, retry=0, qname=None):
        ...
        """
        detail = {}
        headers = {"referer": url}#"http://gsxt.gdgs.gov.cn/aiccips/CheckEntContext/showInfo.html"}
        result = None
        res = None
        retry_req = 0
        code = 0
        while True:
            ...
            if res is not None:
                ...
                if code == 521:
                    break
                elif code != 200:#code == 502 or code == 503:
                    print "code=", code, "proxy:", self.proxies#, "ua=", self.ua
                    time.sleep(random.randrange(1, 5, 1))
                    continue
                break
            #if retry_req > 5:
        ...
        if code == 521:
            ...
            try:
                ...
                #print "cookie--->", cookie
                self.add_cookie_line('gsxt.gzaic.gov.cn', cookie)
                self.add_cookie_line("gsxt.gzaic.gov.cn", jsluids[random.randrange(0, 3, 1)])
            except Exception as e:
                print "运行node.js出现异常,", e, " res.text:\n", res.text, "获得的文本js:\n", text
                #return False
                return "page_error"
            #url = "http://192.168.2.90:8001/gsxt.gzaic.gov.cn/aiccips/GSpublicity/GSpublicityList.html?service=entInfo_3mKdbX2D+KO3OYKIEVZ8HwrEi13mj9gtknTrIOEalHk=-NGo01vn0hCjN1SeIg0WuiA=="
        ...
        if result is not None:
            ...
            if res2 is None:
                ...
                return "page_error"
            if res2.code == 521:
                return self.get_guangzhou(cnt, cname, url, regist_code, qname=qname)
            ...
            print cnt, "============GET---GUANGZHOU --->", qname, spider.util.utf8str(detail)
            #return True
            return "success"
        else:
            #Log.error("entityShow, regist code = "+ regist_code + ",  cname:"+cname+",result is None" + " ,url = " + url)
...
#######################################################################################
    ...
    def get_GSpublicityList(self, cnt,  cname, url, regist_code, retry=0, qname=None):
        ...
        print cnt, "GSpublicityList--->", qname, spider.util.utf8str(detail)
        #return True
        return "success"
...
if __name__ == '__main__':
    ...
    gsweb.get_guangzhou("", "", "http://gsxt.gzaic.gov.cn/aiccips/GSpublicity/GSpublicityList.html?service=entInfo_3mKdbX2D+KO3OYKIEVZ8HwrEi13mj9gtknTrIOEalHk=-NGo01vn0hCjN1SeIg0WuiA==","")
