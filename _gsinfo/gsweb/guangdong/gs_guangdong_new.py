...
jsluids = ["__jsluid=4d5e0f897bdf48c43f60ebc239d73f74", "__jsluid=fa88f36cca9518759c20dd5b252a8d74", "__jsluid=fbe10b7cd1b72afa796a27bb492459e3"]
...
class SearchGSWebGuangdong(SearchGSWeb):
    ...
    #     uas = []
    #     with open("/home/windy/develop/getjd/_ct_proxy/UA.txt", "r") as f:
    #         for ua in f:
    #             ua = "="+ua
    #             uas.append(ua)
    ...
    def encode_kw(self, kw, dbgdata=None):
        headers = {'Referer': self.info['url']}
        kw = spider.util.utf8str(kw)
        while True:
            self.code = self.solve_image(dbgdata=dbgdata)
            ...
            mm = self.request_url("http://gsxt.gdgs.gov.cn/aiccips/CheckEntContext/checkCode.html",
                        data={"code": self.code, "textfield": kw}, headers=headers, timeout=20)
            if mm is None or mm.code == 502:
                print "验证码校验有误--->", "结果为空." if mm is None else "结果:res.code=%d" % mm.code
                return None
    ...
    def search_company(self, kw):
        try:
            ...
            dbgdata = {}
            kw = self.encode_kw(kw, dbgdata=dbgdata)
            if kw is None:
                return None
            elif kw == "PROXY-ERROR":
                return ["PROXY-ERROR"]
            ...
            si = None
            while True:
                si = self.request_url(infourl, data={"code": self.code, "textfield": kw}, headers=headers, timeout=20)
                if si is None or u'<img src="http://gsxt.gdgs.gov.cn/aiccips//images/controlinfo_new2.gif">' in si.text:
                    print "查询操作过于频繁,请稍后再试...", cname
                    time.sleep(random.randrange(1, 5, 1))
                    continue
                    #return ["OPERATION-FAST"]
                else:
                    break
            if u"暂未查询到相关记录。" in si.text:
                print cname, "暂未查询到相关记录..."
                return ["QUERY-NONE"]
            ...
            for d in dom.xpath("//div[@class='list']"):
                ...
                out.append(oi)
            #print "get############################:", cname, len(out), "----------------->", spider.util.utf8str(out)
            #print json.dumps(out, indent=4, ensure_ascii=0).encode('utf-8')
            #self.run_get_detail(out)
            if "code" in dbgdata and "content" in dbgdata:
                self.save_pic_to_bin(dbgdata["code"], dbgdata["content"])
            return out
        except Exception as e:
            print "查询公司列表出现异常:", e
            return None

    def save_pic_to_bin(self, code, content):
        """将验证码存入bin文件"""
        fn = '%s.%d' % (code, int(time.time()))
        self.saver.pic.append(fn, content)
    ...
    def get_QyxyDetail(self, cnt,  cname, url, regist_code, retry=0, tid=-1):
        ...
        text1 = None
        while True:
            res = self.request_url(url, timeout=30)
            if res is None or res.code != 200:
                ...
                continue
            try:
                text1 = res.content.decode('gb18030').encode('utf-8')
            except Exception as e:
                print e, " 转码错误 "#, res.content
                #return False
                return "code_error"
            if "/App_Themes/Default/Images/error.jpg" in text1:
                print cname, "url=", url, "返回的页面错误..."
                #return False
                return "return_error"
            if "您的查询过于频繁，请稍候再查！" in text1 or "您的查询间隔时间过短，请稍候再查！" in text1:
                #TODO 可能是被封了!
                print "深圳系统：您的查询过于频繁，请稍候再查 --- >此IP可能已经被封..."
                #return False
                time.sleep(random.randrange(1, 5, 1))
                continue
                #return "page_error"
        ...
        if not self.parse_QyxyDetail_basic(text1, regist_code, detail, url, cname, tid=tid):
            return "page_error"

        doc = html.fromstring(text1)
        ...
        data = {"ScriptManager1":"biangengxinxi|Timer2",
                ...
                "":""}
        while True:
            res = self.request_url(url, headers=headers, data=data, timeout=30)
            if res is None:
                print "get_QyxyDetail res is error"
                ...
                break
        text2 = res.content
        text2 = text2.decode('gb18030')
        doc = html.fromstring(text2)
        trs = doc.xpath("//table[@class='detailsList']/tr")
        detail["changesInfo"] = self.parse_changes_info(trs, url_type="QyxyDetail_Changes")
        print cnt, "SSSSSS 深圳系统－－－＞", cname, spider.util.utf8str(detail), url
        self.saver.fs_QyxyDetail.append(spider.util.utf8str(detail))
        self._do_savebin(regist_code, "QyxyDetail", "basic", text1)
        self._do_savebin(regist_code, "QyxyDetail", "changes", text2)
        #return True
        return "success"
...
#############################  GUANGZHOU  new ##########################################################
    ...
    def get_guangzhou(self, cnt, cname, url, regist_code, retry=0, qname=None, wey=0):
        """
        ...
        code = 0
        _jsluid = None
        self.select_user_agent("=Mozilla/5.0 (Windows NT 10.0; WOW64; rv:45.0) Gecko/20100101 Firefox/45.0")
        while True:
            retry_req += 1
            ...
            if res is not None:
                ...
                code = res.code
                if code == 521:
                    m = re.search("Set-Cookie:(.*);", res.headers)
                    if m:
                        _jsluid = m.group(1).strip()
                    # else:
                    #     print "广州系统　获取不到　_jsluid　...　", url
                    #     time.sleep(random.randrange(1, 3, 1))
                    #     setattr(self._curltls, 'cookies', None)
                    #     continue
                    if wey > 5:
                        print wey, "陷入了521的死循环，请检查...", url
                        return "521"
                    break
                if u'<img src="http://gsxt.gzaic.gov.cn/aiccips//images/errorinfo_new2.gif">' in res.text:
                    print cname, "get_guangzhou  The system can't display 系统无法显示", url
                    return "notdisplay"
                if code != 200:#code == 502 or code == 503:
                    print "get_guangzhou code=", code, "proxy:", self.proxies#, "ua=", self.ua
                    time.sleep(random.randrange(1, 5, 1))
                    continue
                break
            #if retry_req > 5:
        ...
        if code == 521:
            ...
            jscode += text
            try:
                __jsl_clearance = spider.util.runjs(jscode)
                #print "cookie--->", cookie
                self.add_cookie_line('gsxt.gzaic.gov.cn', __jsl_clearance)
                if _jsluid is not None:
                    self.add_cookie_line('gsxt.gzaic.gov.cn', _jsluid)
            except Exception as e:
                print "运行node.js出现异常,", e, " res.text:\n", res.text, "获得的文本js:\n", text
                #return False
                return "page_error"
            #url = "http://192.168.2.107:8002/gsxt.gzaic.gov.cn/aiccips/GSpublicity/GSpublicityList.html?service=entInfo_v0aMi7tSLsoEDkCy7V3bR0OgZSlziwda/oQqfoB0GjE=-50aS1uze1DaXd8Gk5PFw0A=="
            return self.get_guangzhou(cnt, cname, url, regist_code, retry=(retry+1), qname=qname, wey=(wey+1))
        elif code == 200:
            if res.text is None:
                if retry < 5:
        ...
        if result is not None:
            ...
            if res2 is None:
                ...
                return "page_error"
            if res2.code == 521:
                return self.get_guangzhou(cnt, cname, url, regist_code, qname=qname, wey=(wey+1))
            ...
            #股东信息
            data = {"pageNo": "1", "entNo": entNo, "regOrg": regOrg}
            headers = {"referer": url, "X-Requested-With": "XMLHttpRequest",
                       "Content-Type": "application/x-www-form-urlencoded; charset=UTF-8"}
            investor_text = None
            while True:
                res = self.request_url("http://gsxt.gzaic.gov.cn/aiccips/GSpublicity/invInfoPage.html", data=data, headers=headers)
                if res is None:
                    continue
                if res.code == 521:
                    return self.get_guangzhou(cnt, cname, url, regist_code, qname=qname, wey=(wey+1))
                if res.code == 200:
                    try:
                        investor_text = res.text.replace(u',null', u',""').replace(u':null', u':""').replace(u'null,', u'"",')
                        #print "股东信息investor_text：", investor_text
                        investors = eval(investor_text)
                        detail["investorsInfo"] = investors
                        break
                    except Exception as e:
                        print "get_guangzhou　获取股东信息出错 ", e, res.text
            ...
            #变更信息
            data = {"pageNo": "1", "entNo": entNo, "regOrg": regOrg, "entType": entType}
            headers = {"referer": url, "X-Requested-With":"XMLHttpRequest", "Content-Type":"application/x-www-form-urlencoded; charset=UTF-8"}
            change_text = None
            while True:
                res = self.request_url("http://gsxt.gzaic.gov.cn/aiccips/GSpublicity/entChaPage",data=data, headers=headers)
                if res is None:
                    continue
                if res.code == 521:
                    return self.get_guangzhou(cnt, cname, url, regist_code, qname=qname, wey=(wey+1))
                if res.code == 200:
                    try:
                        change_text = res.text.replace(u',null', u',""').replace(u'null,', u'"",').replace(u':null', u':""')
                        changes_info = eval(change_text)
                        detail["changesInfo"] = changes_info
                        break
                    except Exception as e:
                        print "get_guangzhou　获取变更信息出错 ", e, res.text
            ...
            self._do_savebin(regist_code, "guangzhou", "investors", investor_text)
            self._do_savebin(regist_code, "guangzhou", "changes", change_text)
            ...
            print cnt, "ZZZZZZ 广州系统－－－＞", qname, spider.util.utf8str(detail)
...
#######################################################################################
    ...
    def req_GSpublicityList(self, cnt, cname, url, **kwargs):
        ...
        while True:
            ...
            if res is not None:
                if u'<img src="http://gsxt.gdgs.gov.cn/aiccips//images/errorinfo_new2.gif">' in res.text or u'<img src="http://gsxt.gzaic.gov.cn/aiccips//images/errorinfo_new2.gif">' in res.text:
                    print cname, "req_GSpublicityList  The system can't display 系统无法显示", url
                    return "notdisplay"

            if res is None or res.code != 200:
                if res is not None and res.code == 521:
                    return res
                print cnt, cname, "广东系统－－－ ", "res is none" if res is None else "res.code = %d" %res.code, ", retry:", timeout_retry, "url=", url
                timeout_retry += 1
                if timeout_retry > 100:
                ...
                if '<img src="http://gsxt.gdgs.gov.cn/aiccips//images/controlinfo_new2.gif">' in res.text:
    ...
    def get_GSpublicityList(self, cnt,  cname, url, regist_code, retry=0, qname=None):
        """
        ...
        print cnt, "DDDDDD 广东系统－－－＞", qname, spider.util.utf8str(detail)
        #return True
        return "success"
...
if __name__ == '__main__':
    ...
    gsweb = SearchGSWebGuangdong(None)
    #gsweb.get_QyxyDetail("","", "http://www.szcredit.com.cn/web/GSZJGSPT/QyxyDetail.aspx?rid=22452b4b64014143a1432e60193d8b7c","")
    ...
    #gsweb.get_guangzhou("", "", "http://gsxt.gzaic.gov.cn/aiccips/GSpublicity/GSpublicityList.html?service=entInfo_v0aMi7tSLsoEDkCy7V3bR0OgZSlziwda/oQqfoB0GjE=-50aS1uze1DaXd8Gk5PFw0A==","")
