...
class GSinfo2DB(Spider):
    """工商数据入库  通用版　广东三个系统及四川湖南都测试过可用　如发现不满足的字段　可自行添加到key_vvalue中
       这个版本不用考虑提取［投资信息］的链接　因为发现链接没用　（从　变更信息　或　主要人员信息　中提取出来的生成链接的参数跟投资信息中的不是同一条）
       之后可能需要将广州和广东的注册码提取出来　到bin文件中提取出页面再进行链接生成和拉取解析．
    """
    ...
    def dispatch(self):
        ...
        #fl = "./gsweb/guangdong/gsinfo_guangdong_guangzhou.txt"
        files = ["./gsweb/guangdong/gsinfo_guangdong_GSpublicityList.txt", "./gsweb/guangdong/gsinfo_guangdong_QyxyDetail.txt",
                 "./gsweb/guangdong/gsinfo_guangdong_entityShow.txt", "./gsweb/guangdong/gsinfo_guangdong_guangzhou.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_GSpublicityList_gz.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_GSpublicityList_gz1.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_GSpublicityList_gz3.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_guangzhou_gz1.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_guangzhou_gz2.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_guangzhou_gz3.txt",
                 "./gsweb/guangdong/guangzhou/gsinfo_guangdong_QyxyDetail_gz1.txt",
                 "./gsweb/hunan/gsinfo_hunan.txt", "./gsweb/sichuan/gsinfo_sichuan.txt"]
        for fl in files:
            with open(fl) as f:
                for line in f:
                    line = line.strip()
    ...
    def parse(self, detail):
        ...
        detail_new = {}
        registID = self.parse_basic(basic, detail, detail_new)

        if len(branch) != 0:
           self.parse_branch(branch, detail_new)
        else:
            detail_new["branchInfo"] = []

        if len(changes) != 0:
            self.parse_change(changes, detail_new, registID=registID)
        else:
            detail_new["altInfo"] = []

        if len(staffs) != 0:
            self.parse_staff(staffs, detail_new, registID=registID)
        else:
            detail_new["staffsInfo"] = []

        #解析投资信息一定要放在最后
        if len(invest) != 0:
            self.parse_invest(invest, detail_new)
        else:
            detail_new["investInfo"] = []

        self.save2db(registID, detail_new)
        #print "解析结果：", spider.util.utf8str(detail_new)
    ...
    def parse_staff(self, staffs, detail_new, registID=None):
        ...
        for s in staffs:
            ...
            #self.get_invest_detail_guangdong(s, registID=registID)
        detail_new["staffsInfo"] = staff_new
        #print "主要人员信息：", spider.util.utf8str(s_new)

    def get_invest_detail_guangdong(self, staff, registID):
        ...
        if "entNO" in staff and "vipNO" in staff:
            ...
            self.invest_detail_url.append(registID + "," + url)



    def parse_change(self, changes, detail_new, registID=None):
        ...
        for c in changes:
            ...
            #self.get_invest_detail_guangzhou(c, registID=registID)
        #print "变更信息：", spider.util.utf8str(alt_new)
        detail_new["altInfo"] = alt_new


    def get_invest_detail_guangzhou(self, change, registID=None):
    ...
    def parse_branch(self, branch, detail_new):
    ...
    def save2db(self, registID, detail):
        """入库 -- 先取出信息与当前信息对比　哪个更全用哪个"""
        db = conn.gsinfo
        detail_db = db.gongshang_info.find_one({"registID": "1234567890"})
        detail_in = None
        if detail_db is not None:
            detail_in = self.check_is_all(detail_db, detail)
        else:
            detail_in = detail
        key = {"registID": registID}
        result = db.gongshang_info.update(key, detail_in, upsert=True)
        #{'updatedExisting': True, u'nModified': 1, u'ok': 1, u'n': 1}
        #{'updatedExisting': False, u'nModified': 0, u'ok': 1, u'upserted': ObjectId('573fc9135e0eaac40232cffd'), u'n': 1}
        if isinstance(result, dict):
            self.all_count += 1
            if result["updatedExisting"]:
                self.update_count += 1
                print registID, " 更新成功...", self.update_count
            else:
                self.new_count += 1
                print registID, "新数据入库成功...", self.new_count
        else:
            self.fail_count += 1
            print "入库失败＞>>>>>>>>．．．", result, self.fail_count


    def check_is_all(self, detail_db, detail):
        """ compared to the current detail and the DB's detail """
        detail_in = {}
        basicInfodb = detail_db["basicInfo"]
        basicInfo = detail["basicInfo"]
        if len(basicInfo) < len(basicInfodb):
            detail_in["basicInfo"] = basicInfodb
        else:
            detail_in["basicInfo"] = basicInfo

        branchInfodb = detail_db["branchInfo"]
        branchInfo = detail["branchInfo"]
        if len(branchInfo) < len(branchInfodb):
            detail_in["branchInfo"] = branchInfodb
        else:
            detail_in["branchInfo"] = branchInfo

        investInfodb = detail_db["investInfo"]
        investInfo = detail["investInfo"]
        if len(investInfo) < len(investInfodb):
            detail_in["investInfo"] = investInfodb
        else:
            detail_in["investInfo"] = investInfo

        altInfodb = detail_db["altInfo"]
        altInfo = detail["altInfo"]
        if len(altInfo) < len(altInfodb):
            detail_in["altInfo"] = altInfodb
        else:
            detail_in["altInfo"] = altInfo

        staffsInfodb = detail_db["staffsInfo"]
        staffsInfo = detail["staffsInfo"]
        if len(staffsInfo) < len(staffsInfodb):
            detail_in["staffsInfo"] = staffsInfodb
        else:
            detail_in["staffsInfo"] = staffsInfo
