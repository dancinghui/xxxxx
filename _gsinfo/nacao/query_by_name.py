...
class Nacao(Spider):
    def __init__(self, thcnt):
        ...
        if self.proxy_mode == 0:
            ...
            Spider.__init__(self, 100)#len(self.proxies_dict))
            ...
            self.proxies_dict = [#{'http': 'http://ipin:helloipin@183.56.160.174:50001', 'https': 'https://ipin:helloipin@183.56.160.174:50001'},
                                ...
                                #{'http': 'http://ipin:helloipin@106.75.134.189:18889', 'https': 'https://ipin:helloipin@106.75.134.189:18889'},
                                {'http': 'http://ipin:helloipin@192.168.1.39:3428', 'https': 'https://ipin:helloipin@192.168.1.39:3428'}]
        ...
        self.saver = FileSaver("nacao_queries_info_local.txt")
        self.already = FileSaver("nacao_queries_already_local.txt")
        self.queries0 = FileSaver("nacao_queies_0_local.txt")
    ...
    def get_adsl_proxies(self):
        proxies = {'http': 'http://ipin:ipin1234@106.75.134.189:18888', 'https': 'https://ipin:ipin1234@106.75.134.189:18888'}
        ...
        while True:
            ...
            if res is not None and res.code == 200:
                ...
                for k, v in result.items():
                    v = "ipin:helloipin@" + v
                    proxies1 = self._match_proxy(v.encode("utf-8"))
                    con = self.request_url("https://s.nacao.org.cn/servlet/ValidateCode?time=1465267840016", proxies=proxies1)
                    print "None" if con is None else con.code
                time.sleep(1)
                continue
            else:
    ...
    def init_already(self):
        with open("nacao_queries_already_local.txt", "r") as f:
            cnt = 0
            for line in f:
                cnt += 1
                line = line.strip()
    ...
    def dispatch(self):
        with open("r1_fenci_local_filter.txt", "r") as f:
            cnt = 0
            for line in f:
                line = line.strip()
                cnt += 1
                if line in filter_kw:
                    print cnt, "keywords [ %s ] already queried !" % line
                    continue
                job = {"kw": line, "cnt": cnt, "retry": 0}
    ...
    def run_job(self, job):
        ...
        self.search(kw, proxies=proxies, cnt=cnt, retry=retry)
    ...
    def search(self, kw, proxies={}, cnt=0, retry=0):
        ...
        keyword = kw.replace("&", "").replace("*", "").replace("#", "")
        data = self.gen_post_data(kw=keyword)#kw)
        res = None
        while True:
        ...
        if u"请求无效或页面没找到" in res.text:
            self.already.append(kw)
            print cnt, kw, "请求无效或页面没找到......"
            return
        try:
            r = eval(res.text)
            result = r[1]
            print cnt, kw, "查询到", len(result), "条数据...", spider.util.utf8str(result)
            self.speed_record(proxies=proxies)
            if len(result) == 0:
                #查询结果为空的关键字都写入到同一个文本
                self.queries0.append(kw)
            else:
                for t in result:
                    self.saver.append(spider.util.utf8str(t))
            self.already.append(kw)
        except Exception as e:
            print cnt, kw, "数据查询结果转换出错,", e, "res.text:\n", res.text
