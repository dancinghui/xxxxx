...
class QycxbQuery(Spider):
    ...
    def req_all(self, encryptedJson, retry=0, cname=None):
        ...
        res = None
        if self.is_debug:
            res = self.request_url(url, headers={"Content-Type": "application/json", "Accept-Language": "zh-Hans-CN;q=1"}, timeout=20, data=param, proxies={'http': 'http://ipin:helloipin@192.168.1.39:3428', 'https': 'https://ipin:helloipin@192.168.1.39:3428'})
        else:
