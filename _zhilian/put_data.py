...
class DisJDZLPageStore(zhilian.ZhilianPageStore):
...
class JDZLBin2DB(Bin2DB):
    def __init__(self):
        Bin2DB.__init__(self)
        self.pagestore = DisJDZLPageStore()

    def parse_name(self, n):
        m = re.match(r'jd_zhilian\.(\d+)\.(\d+)', n)
        if m:
    ...
    def get_url(self, jdid):
        url = "http://jobs.zhaopin.com/%s.htm" % jdid
        return url
