...
fixed = ['学校名称','省份','类型']
pfcolumn = ['招生批次','专业代码','专业名称','计划数','学制','学费']

class YouzySpiderNP(Spider):
    def __init__(self,thcnt):
        Spider.__init__(self, thcnt)
        self.sessionReq = YouzyLogin()
        self.sessionReq.do_login(6)
        self.savefile=CsvSaver("spider_zhuanke_np.csv",fixed+pfcolumn)
        self.__fail_urls = FileSaver("spider_zhuanke_fail_np.txt")
        self.id_count = 0
        self.url_count = 0
        self.parse_count = 0
    ...
    def run_job(self, jobid):
        ...
        id = jobid.get("id")
        baseUrl = "http://www.youzy.cn/college/newplan?Id="+id;
...
if __name__ == "__main__":
    ...
    print "youzy_np_link.py is finished , id_count : {}, url_count:{},parse_count:{}".format(s.id_count , s.url_count ,s.parse_count)
