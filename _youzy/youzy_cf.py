...
class YouzyCFSpider(Spider):

    def __init__(self,thcnt):
        Spider.__init__(self, thcnt)
        self.sessionReq = YouzyLogin()
        self.sessionReq.do_login(7)
        self.num_count = 0
        self.parse_count = 0
        self.url_count = 0
        self.savefile=CsvSaver("spider_url_zhuanke_cf.csv",fixed+pfcolumn)
        self.__fail_urls = FileSaver("spider_url_fail_zhuanke_cf.txt")
    ...
    def dispatch(self):
        #f = open("spider_fail_cf.txt", "r")
        f = open("spider_zhuanke_fail_cf.txt", "r")
        while True :
    ...
    def run_job(self, jobid):
        ...
        if res is None or res.code == 404:
            ...
            if url.__contains__('codeId'):
                if url.__contains__('batch'):
                    retry = int(jobid.get("retry"))
                    self.parse(res.content,url,retry)
                else:
                    doc = html.fromstring(res.content)
                    values = doc.xpath('//select[@id="ddlBatch"]/option/@value')
                    if values is not None and len(values) > 0:
                        for value in values :
                            url2 = url + "&batch="+value
                            jobid = {"url":url2,"retry":0}
                            self.add_job(jobid)
                    else :
                        retry = int(jobid.get("retry"))
                        self.parse(res.content,url,retry)
            elif url.__contains__('batch'):
                retry = int(jobid.get("retry"))
                self.parse(res.content,url,retry)
            else:
                doc = html.fromstring(res.content)
